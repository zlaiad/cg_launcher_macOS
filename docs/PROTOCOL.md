...
分析对象为用户已安装的 `POLCN_Launcher.exe`、`cg_se_3000.exe` 和官方 XML 配置。只在用户这台机器实现官方账号正常登录与游戏启动，不修改官方程序，不伪造服务端认证结果。官方二进制副本、反汇编中间文件与工具曾放在被 Git 忽略的 `work/`，已在项目整理时清理；关键结论、文件哈希和分析脚本保留在文档及 `scripts/`。官方文件与临时分析产物不放入应用包或 Git 仓库。
...
令牌字段与官方回调的 C 字节串行为一致：只采用第一个 NUL 结束符之前的原始字节，允许非 ASCII 字节，不做 Unicode 解码或重新编码。末尾整数按 UInt32 加 `0x80000000` 后取模，再转小写 32 进制，与原程序的 32 位字段及 `__ui64toa` 调用一致。共享内存由自有 `cg_bridge.exe` 建立，再调用 `CreateProcessW` 启动原版游戏。桥接进程随游戏存活以保持映射。
...
用户遇到“字段 32 字节，文本 32 字节”的交接错误。旧代码虽然允许 32 字节，却要求令牌中的每个字节都属于受限可打印 ASCII；冒号、空白、控制字节及高位字节均会被拒绝。这是早期实现遗留限制，0.3 的界面/更新修改没有改变该逻辑。

再次核对成功回调 `0x4133a0`：将响应令牌逐字节复制到 33 字节数组，再追加 NUL，没有字符编码转换。0.3.1 因此使用 `Data` 组装交接字段，通过长度前缀 stdin 传递；路径及普通启动参数仍使用 UTF-8。桥接程序原本即使用 `char*` 和 `memcpy` 写共享内存，生产交接路径无需转换令牌。继续保留 32 字节上限、首个 NUL 截断、空值检查、账号归属和会话有效期检查。

新增覆盖全部 255 种非 NUL 字节的合成令牌测试，以及服务端响应解析到 stdin 字段的逐字节一致性检查。Wine 测试使用独立命名共享内存，将包含 32 个非 UTF-8 字节的合成票据从 stdin 写入共享内存，再由子进程逐字节核对；不接触真实游戏认证共享内存，也不记录真实令牌。
